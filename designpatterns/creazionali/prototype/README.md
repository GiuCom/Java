...
## 🚀 Introduzione
Il pattern **Prototype** è un design pattern creazionale che permette la creazione di nuovi oggetti clonando un'istanza esistente, chiamata appunto "prototipo".<br>
...
- **Prototype:** Interfaccia che dichiara il metodo per la clonazione (es. `clone()` o `copy()`).
...
- **Ruolo:** Implementa l'interfaccia **Cloneable**.
- **Dettaglio:** Contiene il metodo `clone()`.
- **Responsabilità:** Fornire una base comune (`id` e `nomeFigura`) e permettere la duplicazione senza che il client conosca la classe concreta.
...
- **Dettaglio:** Nel costruttore impostano il nome della figura (variabile `nomeFigura`). Non devono necessariamente implementare di nuovo `clone()` a meno che non abbiano oggetti complessi all'interno che richiedono una **Deep Copy**.
- **Utilità:** Sono i modelli reali. Se creare un oggetto della classe **Cerchio** richiedesse molto tempo (es. caricamento di coordinate complesse), clonarne un oggetto già istanziato rende il processo molto più veloce.
...
- **Flusso:** Invece di scrivere `new Cerchio()`, invoca il metodo di clonazione `.clone()`.
...
- **Uso:** Il client riceve la copia e la modifica (es. cambia l'`id`) senza rovinare l'oggetto master originale.

<br>

### Prototype Registry
A questo esempio possiamo aggiungere la classe **FiguraCache** (Prototype Registry) che funge da catalogo centrale. Invece di creare manualmente ogni prototipo nel `main`, il Prototype Registry memorizza un set di oggetti pre-configurati in una `Map`
...
- **Performance:** Gli oggetti "costosi" da creare, dal punto di vista computazionale, vengono generati una sola volta all'avvio e poi clonati all'infinito.
...
La classe **PrototypeMain.java** possiamo modificarla per utilizzare la classe **FiguraCache.java**<br>
Il client nel pattern **Prototype** con Prototype Registry non è più un semplice creatore di oggetti, ma un consumatore di servizi. Il suo compito principale è richiedere copie di oggetti pre-configurati senza conoscere i dettagli della loro implementazione o del loro costo di creazione.
...
Non saprà a quale figura corrisponde al `id` "1" o "2" (un cerchio, un rettangolo o un'altra sottoclasse). Questo riduce drasticamente l'accoppiamento (**dependency**) tra il codice del `main` e le classi concrete.
```java
Figura clonedFigura1 = FiguraCache.getFigura("1");
...
Pertanto, è possibile personalizzare la copia esatta del prototipo poiché il Registry ha restituito un clone, la modifica del `id` con il valore "100-Nuovo" avviene solo sull'istanza locale del `main`. L'oggetto originale dentro la classe **FiguraCache** rimane intatto con il suo `id` originale ("1").
```java
clonedFigura1.setId("100-Nuovo");
```

Per verificare l'indipendenza il `main` stampa entrambi i valori della variabile `id` per dimostrare che l'originale in cache non è stato corrotto.<br>
...
- `assertNotSame(obj1, obj2)`: test importante per il pattern **Prototype**. Verifica che `obj1 == obj2` sia falso. Se fallisse, significherebbe che stai usando lo stesso oggetto e non un clone.
...
- **Shallow Copy** (Default di **Java**): Il metodo `super.clone()` copia i valori dei campi. Se un campo è un riferimento a un oggetto (es. un `Color` o una `List`), viene copiato l'indirizzo di memoria. Si ottiene che l'originale e il clone puntano allo stesso sotto-oggetto. Se il clone modifica quell'oggetto, l'originale cambia di conseguenza.
- **Deep Copy**: Il metodo `clone()` viene istruito per clonare ricorsivamente anche tutti gli oggetti contenuti. Si ottiene che l'originale e il clone sono totalmente indipendenti, inclusi i loro componenti interni.
...
Per implementare correttamente la **Deep Copy** (copia profonda), la classe **Figura** è stata trasformata da un semplice contenitore di dati a una classe capace di gestire la propria gerarchia di dipendenze.<br>
Nella nuova versione, abbiamo aggiunto:
```java
protected Coordinate coordinate; // Oggetto annidato
```
In Java, se cloniamo **Figura** usando il metodo standard, la variabile `coordinate` copierà solo l'indirizzo di memoria. Sia l'originale che il clone punterebbero allo stesso identico oggetto **Coordinate** fisico.
Quindi, la modifica più critica, è nel corpo del metodo `clone()`. Non ci limitiamo più a chiamare `super.clone()`, chiamata nativa che crea l'istanza del clone e copia i campi `id` e `nomeFigura`. Essendo stringhe (immutabili), la copia superficiale è sicura. Tuttavia, per il campo `coordinate`, copia il puntatore.<br>
Il seguente codice:
...
è il cuore della modifica. Stiamo dicendo al clone: "Non usare la mia posizione, creane una nuova che sia la copia della mia".<br>
Così facendo, i due oggetti "originale" e "clonato" otterrebbero dalla riga: 
```java
...
Il blocco `try-catch`, sebbene la classe **Figura** implementi **Cloneable**, è buona norma gestire l'eccezione internamente per pulire la firma del metodo, permettendo al Registry o al `main` di chiamare `.clone()` senza dover gestire eccezioni checked ogni volta.<br>
Aggiungendo i metodi `setPosition` e `getPosition`, abbiamo dato al `main` la possibilità di testare lo stato. Senza la **Deep Copy**, un'istruzione come `clonato.setCoordinate(500,200)` avrebbe modificato anche l'originale. Grazie alla modifica nella classe **Figura**, il "confine" dell'oggetto è ora protetto.


Analisi dei Pro e Contro del *Deep Copy*
...
## Test
Per testare correttamente la **Deep Copy**, la classe di test **PrototypeTest.java** non deve limitarsi a controllare se i valori (come le coordinate X e Y) sono uguali, ma deve verificare se l'identità degli oggetti annidati è diversa.
...
- `testDeepCopyIdentity`: Utilizza `assertNotSame` sull'oggetto **Coordinate**. Se questo test fallisce, significa che abbiamo ottenuto una **Shallow Copy** (entrambi gli oggetti **Figura** puntano alla stessa istanza di **Coordinate**).
